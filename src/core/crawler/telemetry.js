...
global.crawlerState = global.crawlerState || {
    ...
    activeWorkers: 0,
    concurrencyLimit: 3,  // L3 FIX: Match actual BASE_CONCURRENCY in engine.js
    startTime: Date.now(),
    lastAction: Date.now(),
...
export async function logGuardianEvent(mangaId, chapterTitle, eventType, message) {
    try {
        ...
        await query(`
            INSERT INTO guardianreports (manga_id, chapter_title, event_type, message, created_at)
            VALUES (@mangaId, @chap, @type, @msg, GETDATE())
        `, {
            mangaId: mangaId || 'System',
