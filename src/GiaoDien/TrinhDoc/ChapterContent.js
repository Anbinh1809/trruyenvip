...
import RecrawlButton from '@/GiaoDien/ThanhPhan/RecrawlButton';
import { Link as LinkIcon, AlertCircle, RefreshCw } from 'lucide-react';
import { useReaderKeys } from '@/TroThu/Hooks/useReaderKeys';
    ...
    const startPolling = useCallback(() => {
        ...
        pollInterval.current = setInterval(async () => {
            ...
            if (retries > MAX_RETRIES) {
                stopPolling();
                setIsSyncing(false);
                setError('Hệ tho‘ng cào Ä‘ang bận hoáº·c nguồn không phản hồi. Vui lòng bấm Thử lại.');
                return;
            }
    ...
    const startSync = useCallback(async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                setError(data.error || 'Hệ tho‘ng Ä‘ang bảo trà¬ hoáº·c nguồn không phản hồi.');
                setIsSyncing(false);
            }
        } catch (e) {
            setError('Lỗi kết nối máy chủ. Vui lòng thử láº¡i.');
            setIsSyncing(false);
        }
        ...
        return (
            <div className="syncing-overlay-industrial fade-in">
                ...
                <h3 className="sync-title-industrial">
                    <LinkIcon size={24} color="var(--accent)" /> ÄANG ÄỒNG Bộ Do® LIộU...
                </h3>
                <p className="sync-sub-industrial">
                    Hệ tho‘ng Ä‘ang trích xuất nội dung trựcc tiáº¿p từ nguồn chà­nh cho báº¡n. Quá trà¬nh nà y thưong máº¥t khoảng 3-8 giây.
                </p>
        ...
        <div className="reader-img-container fade-in">
            ...
            {images.length === 0 && !isSyncing && (
                <div className="empty-reader-state industrial-p-80">
                    <p className="empty-desc-industrial">Chương nà y hiện không tà¬m tháº¥y dữ liệu ảnh.</p>
                    <div className="mt-30">
    ...
    if (error) {
        return (
            <div className="reader-img-error-card industrial-error-nebula min-h-300">
                ...
                <p className="mt-20">Trang {idx + 1} máº¥t kết nối.</p>
