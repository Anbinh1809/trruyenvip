...
export default function LiveSearch({ onSelect }) {
  ...
  const handleMigration = async () => {
      ...
      try {
          ...
          const data = await res.json();
          if (data.success) {
              if (addToast) addToast('Äồnng bo™ dữ liệu thành công!', 'success');
              if (onSelect) onSelect();
                ...
                {isUrl && (
                    <div className="titan-migration-bar-industrial">
                        <button onClick={handleMigration} className="btn btn-primary migration-btn-titan">
                            <Zap size={14} fill="currentColor" /> {loading ? 'ÄANG ÄỒNG Bộ...' : 'ĐỒNG DỮ LIỆU NGUỒN NGOÀI'}
                        </button>
                    </div>
                )}
                        ...
                        results.map((m, idx) => (
                            ...
                            >
                                ...
                                <div className="result-info-titan-industrial">
                                    ...
                                    <div className="result-sub-titan-industrial truncate-1">{m.author || 'Äang cập nhật'}</div>
                                </div>
