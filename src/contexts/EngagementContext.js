...
import LevelUpOverlay from '@/components/widgets/LevelUpOverlay';
import { useAuth } from './AuthContext';

import { RANKS, MISSION_TYPES, CHEST_DATA } from '@/core/constants/engagement';

const EngagementContext = createContext();

export { RANKS, calculateRank } from '@/core/constants/engagement';
...
function engagementReducer(state, action) {
  switch (action.type) {
    case 'SYNC_USER': {
        const { xp, vipCoins, uuid, missionData } = action.payload;
...
export function EngagementProvider({ children }) {
  ...
  }, [isAuthenticated, uXp, uCoins, uUuid, missionData]);
  ...
  useEffect(() => {
    ...
    checkDateChange();
    const timer = setInterval(checkDateChange, 300000); 
