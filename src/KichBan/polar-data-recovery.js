...
async function performPolarRecovery() {
    ...
    try {
        ...
        await query("UPDATE manga SET author = 'Äang cáº­p nháº­t' WHERE author IS NULL OR author = '' OR author = '??'");
        await query("UPDATE manga SET status = 'Äang tiáº¿n hÃ nh' WHERE status IS NULL OR status = '' OR status = '??'");
