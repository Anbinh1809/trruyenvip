...
export default async function proxy(request) {
  ...
  if (isSlightlyProtected && !pathname.startsWith('/api/proxy')) {
      ...
      if (userLog.count > MAX_REQUESTS) {
          return new NextResponse(
              JSON.stringify({ error: 'Bạn thao tác quá nhanh! Vui lòng đợi trong giây lát.' }), //tui đang làm 1 số thứ ko ổn
  ...
  const protectedPaths = ['/profile', '/favorites', '/history', '/rewards', '/leaderboard'];
  const adminPaths = ['/admin', '/api/admin', '/api/crawler/stats'];
