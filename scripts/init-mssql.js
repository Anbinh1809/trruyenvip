...
async function init() {
    console.log('--- Initializing TruyenVip SQL Server Schema ---');
    try {
        await query(`
        ...
        await query(`
            IF COL_LENGTH('users', 'mission_data') IS NULL
            ALTER TABLE users ADD mission_data NVARCHAR(MAX) DEFAULT '{}';
        `);
        await query(`
            IF COL_LENGTH('users', 'last_mission_reset') IS NULL
            ALTER TABLE users ADD last_mission_reset DATETIME2 DEFAULT GETDATE();
        `);
        ...
        await query(`
            IF COL_LENGTH('chapters', 'status') IS NULL
            ALTER TABLE chapters ADD status NVARCHAR(20) DEFAULT 'pending';
        `);
        await query(`
            IF COL_LENGTH('chapters', 'fail_count') IS NULL
            ALTER TABLE chapters ADD fail_count INT DEFAULT 0;
        `);
        ...
        // SQL Server Indexes: Note that Full Text Search is usually preferred over GIN trgm but we create normal indices for now.
        await query(`IF NOT EXISTS(SELECT * FROM sys.indexes WHERE name = 'idx_chapters_manga_id') CREATE INDEX idx_chapters_manga_id ON chapters(manga_id);`);
        await query(`IF NOT EXISTS(SELECT * FROM sys.indexes WHERE name = 'idx_chapters_updated_at') CREATE INDEX idx_chapters_updated_at ON chapters(updated_at DESC);`);
        await query(`IF NOT EXISTS(SELECT * FROM sys.indexes WHERE name = 'idx_users_xp') CREATE INDEX idx_users_xp ON users(xp DESC);`);
        await query(`IF NOT EXISTS(SELECT * FROM sys.indexes WHERE name = 'idx_users_vipcoins') CREATE INDEX idx_users_vipcoins ON users(vipcoins DESC);`);
        await query(`IF NOT EXISTS(SELECT * FROM sys.indexes WHERE name = 'idx_manga_views_rating') CREATE INDEX idx_manga_views_rating ON manga(views DESC, rating DESC);`);
        await query(`IF NOT EXISTS(SELECT * FROM sys.indexes WHERE name = 'idx_crawlertasks_status') CREATE INDEX idx_crawlertasks_status ON crawlertasks(status, priority DESC);`);
        await query(`IF NOT EXISTS(SELECT * FROM sys.indexes WHERE name = 'idx_crawlertasks_manga_id') CREATE INDEX idx_crawlertasks_manga_id ON crawlertasks(manga_id);`);
        await query(`IF NOT EXISTS(SELECT * FROM sys.indexes WHERE name = 'idx_comments_chapter_id') CREATE INDEX idx_comments_chapter_id ON comments(chapter_id);`);
        await query(`IF NOT EXISTS(SELECT * FROM sys.indexes WHERE name = 'idx_readhistory_composite') CREATE INDEX idx_readhistory_composite ON readhistory(user_uuid, manga_id);`);
        await query(`IF NOT EXISTS(SELECT * FROM sys.indexes WHERE name = 'idx_crawllogs_created_at') CREATE INDEX idx_crawllogs_created_at ON crawllogs(created_at DESC);`);
        await query(`IF NOT EXISTS(SELECT * FROM sys.indexes WHERE name = 'idx_guardianreports_created_at') CREATE INDEX idx_guardianreports_created_at ON guardianreports(created_at DESC);`);
        await query(`IF NOT EXISTS(SELECT * FROM sys.indexes WHERE name = 'idx_chapterimages_created_at') CREATE INDEX idx_chapterimages_created_at ON chapterimages(created_at DESC);`);

        console.log('--- SQL Server Initialization Complete ---');
    } catch (err) {
